  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        setError(e instanceof Error ? e.message : "Failed to load applied services");
      }
    };
    load();
  }, []);
  ...
  return (
    <section className="space-y-5">
      <div className="rounded-2xl bg-gradient-to-r from-emerald-600 to-teal-500 p-6 text-white shadow-lg">
        <h1 className="text-2xl font-bold">Applied Services</h1>
        <p className="mt-1 text-sm text-emerald-100">Track all services where you have applied.</p>
      </div>
      ...
      {userRole === "EMPLOYER" ? (
        <p className="rounded-xl bg-white p-5 text-sm text-gray-600 shadow">
          Applied services are available for Worker and Mediator roles.
        </p>
      ) : (
        <>
          <div className="rounded-xl bg-white p-4 shadow">
            <input
              ...
              placeholder="Search applied service by name or location"
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {filtered.map((service) => (
              <div key={service._id} className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
                <p className="text-lg font-semibold text-gray-900">{service.subType}</p>
                <p className="mt-1 text-sm text-gray-600">{service.address}</p>
                <p className="mt-2 text-xs font-medium text-gray-500">Status: {service.status}</p>
                <Link
